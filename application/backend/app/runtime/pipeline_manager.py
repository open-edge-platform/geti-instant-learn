...
class PipelineManager:
    ...
    def get_reference_batch(self, project_id: UUID, prompt_type: PromptType) -> tuple[Batch, dict[int, str]] | None:
        ...
        with self._session_factory() as session:
            ...
            # Get prompts with frames - PromptService handles all frame operations
            prompts_with_frames = prompt_svc.get_visual_prompts_with_frames(project_id, prompt_type)

        if not prompts_with_frames:
            logger.info("No valid prompts with frames found: project_id=%s, prompt_type=%s", project_id, prompt_type)
            return None

        samples = []
        for prompt_with_frame in prompts_with_frames:
            try:
                # Convert prompt schema to sample using the loaded frame
                sample = visual_prompt_to_sample(
                    prompt_with_frame.prompt, prompt_with_frame.frame, category_mappings.label_to_category_id
                )
                samples.append(sample)
            except Exception as e:
                logger.warning(
                    "Failed to convert prompt to sample: prompt_id=%s, error=%s", prompt_with_frame.prompt.id, e
                )
                continue
