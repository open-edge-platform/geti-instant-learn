...
The `Processor` runs on its own worker thread and is the only component that knows when the underlying `ModelHandler`
is actually being initialized vs. ready to predict. It calls into a `ModelStatusReporter` to surface those
