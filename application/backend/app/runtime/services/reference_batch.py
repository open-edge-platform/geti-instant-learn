...
def visual_prompt_to_sample(  # noqa: C901
    ...
    for label_id, polygons in sorted(label_groups.items(), key=lambda x: str(x[0])):
        ...
        if output_bboxes:
            # One bbox per polygon — SAM3 encodes each box as a separate prompt
            ...
            # Merge all polygons into one semantic mask per label
        ...
        # One shot per image-label pair regardless of instance count
...
class ReferenceBatchService:
    ...
    def _build_visual_batch(
        ...
        for prompt in db_prompts:
            ...
            try:
                ...
                sample = visual_prompt_to_sample(
                    prompt=prompt,
                    frame=frame_rgb,
                    label_to_category_id=category_mappings.label_to_category_id,
        ...
        shots_per_category = {
            cat_id: label_shot_counts.get(label_id, 0)
            for label_id, cat_id in category_mappings.label_to_category_id.items()
        }
        logger.info(
