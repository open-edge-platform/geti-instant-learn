...
def test_create_project_default_prompt_mode(service, repo_mock, session_mock):
    """New project gets the default prompt_mode from ProjectConfig."""
    data = ProjectCreateSchema(name="alpha")
    repo_mock.get_active.return_value = None

    result = service.create_project(data)

    assert result.config.prompt_mode == "visual"  # ProjectConfig default


def test_create_project_explicit_prompt_mode_text(service, repo_mock, session_mock):
    """Explicit prompt_mode=text is persisted on project creation."""
    from domain.services.schemas.project import PromptMode

    data = ProjectCreateSchema(name="text_proj", config={"device": "cpu", "prompt_mode": PromptMode.TEXT})
    repo_mock.get_active.return_value = None

    result = service.create_project(data)

    assert result.config.prompt_mode == PromptMode.TEXT


def test_update_project_prompt_mode_to_text(service, repo_mock, session_mock):
    """PUT config.prompt_mode switches the mode from visual to text."""
    from domain.services.schemas.project import PromptMode

    pid = uuid.uuid4()
    existing = make_project(project_id=pid, name="proj")
    existing.config = {"device": "cpu", "prompt_mode": "visual"}
    repo_mock.get_by_id.return_value = existing
    repo_mock.update.return_value = existing

    data = ProjectUpdateSchema(config={"device": "cpu", "prompt_mode": PromptMode.TEXT})
    updated = service.update_project(pid, data)

    assert updated.config.prompt_mode == PromptMode.TEXT
    assert existing.config["prompt_mode"] == "text"
    session_mock.commit.assert_called_once()


def test_update_project_prompt_mode_to_visual(service, repo_mock, session_mock):
    """PUT config.prompt_mode switches the mode from text to visual."""
    from domain.services.schemas.project import PromptMode

    pid = uuid.uuid4()
    existing = make_project(project_id=pid, name="proj")
    existing.config = {"device": "cpu", "prompt_mode": "text"}
    repo_mock.get_by_id.return_value = existing
    repo_mock.update.return_value = existing

    data = ProjectUpdateSchema(config={"device": "cpu", "prompt_mode": PromptMode.VISUAL})
    updated = service.update_project(pid, data)

    assert updated.config.prompt_mode == PromptMode.VISUAL
    assert existing.config["prompt_mode"] == "visual"
    session_mock.commit.assert_called_once()


def test_update_project_prompt_mode_only_does_not_reset_device(service, repo_mock, session_mock):
    """Updating only prompt_mode leaves the device value untouched."""
    from domain.services.schemas.project import PromptMode

    pid = uuid.uuid4()
    existing = make_project(project_id=pid, name="proj")
    existing.config = {"device": "xpu", "prompt_mode": "visual"}
    repo_mock.get_by_id.return_value = existing
    repo_mock.update.return_value = existing

    data = ProjectUpdateSchema(config={"device": "xpu", "prompt_mode": PromptMode.TEXT})
    updated = service.update_project(pid, data)
...
def test_update_project_device_only_does_not_reset_prompt_mode(service, repo_mock, session_mock):
    """Updating only device leaves the prompt_mode value untouched."""
    ...
    # device updated; prompt_mode still "text" from the raw dict
    assert updated.config.device == "cuda"
    assert existing.config["prompt_mode"] == "text"


def test_get_project_returns_prompt_mode(service, repo_mock):
    """get_project maps prompt_mode from the config JSON blob."""
    from domain.services.schemas.project import PromptMode

    pid = uuid.uuid4()
    project = make_project(project_id=pid)
    project.config = {"device": "cpu", "prompt_mode": "text"}
    repo_mock.get_by_id.return_value = project
...
def test_get_project_missing_prompt_mode_falls_back_to_default(service, repo_mock):
    """Projects without prompt_mode in their JSON blob fall back to the Pydantic default."""
    from domain.services.schemas.project import PromptMode

    pid = uuid.uuid4()
    project = make_project(project_id=pid)
    project.config = {"device": "cpu"}  # legacy row – no prompt_mode key
    repo_mock.get_by_id.return_value = project

    result = service.get_project(pid)

    assert result.config.prompt_mode == PromptMode.VISUAL  # default from ProjectConfig


def test_list_projects_returns_prompt_mode(service, repo_mock):
    """list_projects propagates prompt_mode for each project."""
    from domain.services.schemas.project import PromptMode

    p1 = make_project(name="p1")
    p1.config = {"device": "cpu", "prompt_mode": "text"}
    p2 = make_project(name="p2")
    p2.config = {"device": "cuda", "prompt_mode": "visual"}
    repo_mock.list_with_pagination.return_value = ([p1, p2], 2)

    result = service.list_projects()

    modes = {p.name: p.config.prompt_mode for p in result.projects}
    assert modes["p1"] == PromptMode.TEXT
    assert modes["p2"] == PromptMode.VISUAL
