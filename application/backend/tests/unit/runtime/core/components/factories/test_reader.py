...
class TestStreamReaderFactoryValidateConfig:
    """Tests for the validate_config factory method."""

    def test_validate_config_valid_video_file(self, tmp_path: Path) -> None:
        """Test validation succeeds for valid video file config."""
        video_path = tmp_path / "test.mp4"
        video_path.write_bytes(b"fake video content")
        config = VideoFileConfig(source_type=SourceType.VIDEO_FILE, video_path=str(video_path))
        factory = StreamReaderFactory()

        factory.validate_config(config)  # Should not raise

    def test_validate_config_invalid_video_file(self) -> None:
    ...
    def test_validate_config_valid_images_folder(self, tmp_path: Path) -> None:
        """Test validation succeeds for valid images folder config."""
        (tmp_path / "test.jpg").touch()
        config = ImagesFolderConfig(source_type=SourceType.IMAGES_FOLDER, images_folder_path=str(tmp_path))
        factory = StreamReaderFactory()

        factory.validate_config(config)  # Should not raise

    def test_validate_config_invalid_images_folder(self) -> None:
        """Test validation fails for nonexistent images folder."""
        config = ImagesFolderConfig(source_type=SourceType.IMAGES_FOLDER, images_folder_path="/nonexistent/folder")
        factory = StreamReaderFactory()

        with pytest.raises(ValueError, match="Images folder does not exist"):
            factory.validate_config(config)

    def test_validate_config_empty_images_folder(self, tmp_path: Path) -> None:
        """Test validation fails for empty images folder."""
    ...
    def test_validate_config_usb_camera_does_not_raise(self) -> None:
        """Test validation for USB camera config passes (no path validation needed)."""
        config = UsbCameraConfig(source_type=SourceType.USB_CAMERA, device_id=0)
        factory = StreamReaderFactory()
