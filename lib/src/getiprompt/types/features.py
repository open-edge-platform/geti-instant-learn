...
class Features(Data):
    ...
    def get_local_features(
        ...
        If cluster_id is None, return all local features for the class.
        ...
        return [
            f
            for f, m in zip(self._local_features[class_idx], self._meta_data, strict=False)
    ...
    def local_embedding_dim(self) -> int:
        """Get the embedding dimension of the local features."""
        return self._local_features[0][0].shape[-1]

    @property
    def local_features_shape(self) -> int:
