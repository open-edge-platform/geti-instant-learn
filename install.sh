...
GIT_URL="https://github.com/open-edge-platform/geti-instant-learn.git"
GIT_BRANCH="main"
...
Install Intel Geti Instant Learn application and its dependencies.
...
parse_args() {
    VERBOSE=""
    ASSUME_YES=""
    WORK_DIR="$(pwd)/geti-instant-learn"
...
# Can be problematic if Geti installed witha different version of npm or when Geti installed and then folder removed and NVM_DIR pointing to a non-existing files  
install_nvm() {
    export NVM_DIR
...
build_backend() {
    ...
    PYTHONPATH=app "$UV_DIR/uv" run --no-sync openapi.py .build
    cp .build/openapi-spec.json ../ui/src/api/openapi-spec.json
}
...
build_frontend() {
    ...
    run_cmd_spinner "Building API client" "$NPM_BIN" run build:api:types
...
register_shell_cmd() {
...

    {
        echo ""
        echo "$begin_marker"
        echo "function geti-instant-learn { (cd '$WORK_DIR/application/backend' && STATIC_FILES_DIR=html '$UV_DIR/uv' run app/main.py \"\$@\"); }"
        echo "$end_marker"
    } >> "$shell_profile"

    echo "Function 'geti-instant-learn' written to $shell_profile"
...
run_app() {
    echo ""
    echo "Installation complete! Starting Intel Geti Instant Learn..."
    ...
    echo ""
    echo "Geti Instant Learn will be available at: $url"
    echo ""
