...
.PARAMETER WorkDir
    Set the working directory (default: .\geti-instant-learn).
...
param(
    ...
    [Alias("w")]
    [string]$WorkDir = "$(Get-Location)\geti-instant-learn"
)
...
function Build-Backend {
    ...
    try {
        ...
        $env:PYTHONPATH = "app"
        try {
            & $uvExe run --no-sync openapi.py .build
        ...
        Copy-Item -Path ".build/openapi-spec.json" -Destination (Join-Path $uiApiDir "openapi-spec.json") -Force
    } finally {
        Pop-Location
    }
}
...
"@
    ...
    # Create a geti-instant-learn.ps1 PowerShell wrapper
    $ps1Path = Join-Path $WorkDir "geti-instant-learn.ps1"
    $ps1Content = @"
# Intel Geti launcher
...
"@
    ...
    $beginMarker = "# BEGIN Intel Geti Instant Learn"
    $endMarker = "# END Intel Geti Instant Learn"
...
$beginMarker
function geti-instant-learn { Push-Location "$backendDir"; try { `$env:STATIC_FILES_DIR = "html"; & "$uvExe" run app/main.py @args } finally { Pop-Location } }
$endMarker
"@
