...
"""SAM1-style interactive predictor for EfficientSAM3."""
...
from getiprompt.models.foundation.efficientsam3.utils.sam1_utils import SAM2Transforms
...
class SAM3InteractiveImagePredictor:
    ...
    def _prep_prompts(
        ...
        orig_hw = self._orig_hw[img_idx] if isinstance(self._orig_hw, list) else self._orig_hw
    ...
    def set_image(self, image: np.ndarray) -> None:
        ...
        Raises:
            NotImplementedError: If image type is not supported
        """
        ...
        _, vision_feats, _, _ = self.model._prepare_backbone_features(backbone_out)  # noqa: SLF001
    ...
    def _predict(
        ...
        Raises:
            RuntimeError: If no image has been set or if model lacks required components
        """
        ...
        if hasattr(self.model, "sam_prompt_encoder"):
            ...
            msg = "Model does not have sam_prompt_encoder"
            raise RuntimeError(msg)
        ...
        if hasattr(self.model, "sam_mask_decoder"):
            ...
            msg = "Model does not have sam_mask_decoder"
            raise RuntimeError(msg)
    ...
    def predict(
        ...
        Raises:
            RuntimeError: If no image has been set or if predictor is in batch mode
        """
        if not self._is_image_set:
            msg = "An image must be set with .set_image(...) before mask prediction."
            raise RuntimeError(msg)
        ...
        if self._is_batch:
            msg = "This predictor is in batch mode. Use predict_batch for batched inference."
            raise RuntimeError(msg)
    ...
    def predict_batch(
        self,
        point_coords_batch: list[np.ndarray | None] | None = None,
        point_labels_batch: list[np.ndarray | None] | None = None,
        box_batch: list[np.ndarray | None] | None = None,
        mask_input_batch: list[np.ndarray | None] | None = None,
        multimask_output: bool = True,
        return_logits: bool = False,
        normalize_coords: bool = True,
        ...
        Raises:
            RuntimeError: If predictor is not in batch mode
        """
        if not self._is_batch:
            msg = "This predictor is not in batch mode. Use predict for single image inference."
            raise RuntimeError(msg)
