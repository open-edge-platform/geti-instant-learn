...
# Try to import is_tracing - may not be available in older versions
# Note: Dev transformers has is_tracing(tensor) but stable has is_tracing()
try:
    from transformers.utils.import_utils import is_tracing as _is_tracing_orig
    import inspect
    _is_tracing_sig = inspect.signature(_is_tracing_orig)
    if len(_is_tracing_sig.parameters) == 0:
        # Stable transformers version - takes no args
        def is_tracing(tensor=None):
            return _is_tracing_orig()
    else:
        is_tracing = _is_tracing_orig
except ImportError:
    def is_tracing(tensor=None):
        return False
