...
# Compatibility: is_flash_attention_requested may not exist in older transformers versions
try:
    from transformers.utils.generic import is_flash_attention_requested
except ImportError:
    def is_flash_attention_requested(config):
        """Check if flash attention is requested in config."""
        return getattr(config, "_attn_implementation", None) == "flash_attention_2"

# Compatibility: check_model_inputs decorator - use no-op for local Sam3 classes
# The stable transformers version uses _CAN_RECORD_REGISTRY which doesn't support custom classes
def check_model_inputs(func=None, *, tie_last_hidden_states=True):
    """No-op decorator for compatibility with stable transformers.
    
    The check_model_inputs in stable transformers uses _CAN_RECORD_REGISTRY
    which doesn't support custom model classes.
    """
    if func is not None:
        return func
    def decorator(fn):
...
# Stub for auto_docstring that handles both @auto_docstring and @auto_docstring(...)
def auto_docstring(cls=None, **kwargs):
    """Compatibility wrapper for auto_docstring decorator."""
    if cls is not None:
        return cls
    def decorator(cls):
        return cls
...
class Sam3VisionModel(Sam3PreTrainedModel):
    ...
    def __init__(self, config: Sam3VisionConfig):
        super().__init__(config)
        self.config = config
        # Use Sam3ViTModel directly instead of AutoModel.from_config
