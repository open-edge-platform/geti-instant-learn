...
class GetiPromptBatch:
    ...
    def images(self) -> list[torch.Tensor]:
        ...
        if self._images is None:
            self._images = [
                torch.from_numpy(s.image.copy()) if isinstance(s.image, np.ndarray) else s.image
        ...
        if self._masks is None:
            ...
            for s in self.samples:
                if s.masks is not None:
                    mask = (torch.from_numpy(s.masks.copy()) 
                           if isinstance(s.masks, np.ndarray) else s.masks)
                else:
                    mask = None
    ...
    def bboxes(self) -> list[torch.Tensor | None]:
        ...
        for s in self.samples:
            if s.bboxes is not None:
                bbox = (torch.from_numpy(s.bboxes.copy()) 
                       if isinstance(s.bboxes, np.ndarray) else s.bboxes)
            else:
                bbox = None
            result.append(bbox)
        return result
    ...
    def points(self) -> list[torch.Tensor | None]:
        ...
        for s in self.samples:
            if s.points is not None:
                pts = (torch.from_numpy(s.points.copy()) 
                      if isinstance(s.points, np.ndarray) else s.points)
            else:
                pts = None
            result.append(pts)
        return result
    ...
    def category_ids(self) -> list[torch.Tensor]:
        ...
        for s in self.samples:
            if s.category_ids is not None:
                ids = (torch.from_numpy(s.category_ids.copy()) 
                      if isinstance(s.category_ids, np.ndarray) else s.category_ids)
            else:
