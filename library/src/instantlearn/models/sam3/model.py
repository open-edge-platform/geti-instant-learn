...
def _convert_sam3_pt_to_hf_format(state_dict: dict[str, torch.Tensor]) -> dict[str, torch.Tensor]:
    ...
    # Step 3: Fix special shape mismatches
    state_dict = _fix_shape_mismatches(state_dict)
    return state_dict
...
class Sam3Model(nn.Module):
    ...
    def from_pretrained(
        ...
        **kwargs: dict,
        ...
        state_dict = _convert_sam3_pt_to_hf_format(state_dict)
        ...
        # - points_* keys are point encoding (not implemented in current GeometryEncoder)
        tracker_pattern = re.compile(r"^(tracker_model\.|tracker_neck\.)")
        points_pattern = re.compile(r"geometry_encoder\.points_")
        unexpected_keys = [
            k for k in unexpected_keys
            if not tracker_pattern.match(k) and not points_pattern.match(k)
        ]
