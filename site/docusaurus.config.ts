...
// Compute baseUrl: for PR previews use '/<project>/pr-preview/pr-<N>/', else '/<project>/'
const prNumber = process.env.PR_PREVIEW_NUMBER;
const baseUrl = prNumber
  ? `/${projectName}/pr-preview/pr-${prNumber}/`
  : `/${projectName}/`;
...
const config: Config = {
  ...
  // For PR previews, it is '/<projectName>/pr-preview/pr-<N>/'
  baseUrl: baseUrl,
